...
export default function useConversationMsg(conversationId: string) {
  ...
  const loadingOlder = useChatStore(
    (s) => s.loadingOlder[conversationId] ?? false
  );
      ...
      try {
        ...
        if (localMessages.length > 0) {
          ...
          setPaginationState(conversationId, {
            oldestLoadedMessageId: localMessages[0]?.id ?? null,
            hasMore: true,
          });
        }
        ...
        setPaginationState(conversationId, {
          oldestLoadedMessageId: data.nextCursor,
  ...
  const loadOlderMessages = useCallback(async () => {
    ...
    if (!beforeId) {
      setPaginationState(conversationId, { hasMore: false });
      return;
    }

    loadingOlderRef.current = true;
    setPaginationState(conversationId, { loadingOlder: true });
    ...
    try {
      ...
      if (localMessages.length > 0) {
        appendMessages(conversationId, localMessages);
        setPaginationState(conversationId, {
          oldestLoadedMessageId: localMessages[0]?.id ?? beforeId,
        });
        return;
      }
      ...
      setPaginationState(conversationId, { loadingOlder: false });
    }
  }, [
    appendMessages,
    conversationId,
